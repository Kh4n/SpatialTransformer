...
local = layers.Dense(6, kernel_initializer="zeros", bias_initializer=identity_flat)(dense)
# local = layers.Activation("sigmoid")(local)
st = st.SpatialTransformer(name="st")([local, imgs_in])
# out = layers.UpSampling2D(size=(8, 8), interpolation='bilinear')(st)

model = tf.keras.Model(inputs=[imgs_in], outputs=[st])
print(model.summary())
...
history = model.fit_generator(
    ...
    callbacks=[it.ImageOutputAtLayerTensorboard("st", gen[0][0], log_dir='./logs')],
